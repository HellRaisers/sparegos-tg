...
Для сообщения в чат приходит: **кто написал**, **по какому проекту**, **сам
текст сообщения** и **кнопка «Открыть переписку»**. Сообщения известных
клиентов дополнительно помечаются хештегом проекта — см. «Маршрутизация по
клиентам».
...
|---|---|---|
| `Invitation to Interview for: …` | вакансия, условия, описание, личное сообщение клиента | 🎯 `#инвайт` `#интервью` |
| `Invitation to Apply for: …` | вакансия, условия, описание | 📨 `#инвайт` |
...
>   текста в письме нет, поэтому шлём только имя клиента и ссылку.
>
...
- достаёт имя клиента, проект и прямую ссылку на переписку, шлёт в Telegram;
...
- **Тег** добавляется в начало уведомления строкой `🏷 #HillTribe — клиентское
...
- **chat_id** (необязательно) — копия уведомления дополнительно уходит в
  ...
  узнать через @getidsbot (у групп он отрицательный, например `-5159603846`).
