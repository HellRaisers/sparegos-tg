...
Бот мгновенно уведомляет в Telegram о новых сообщениях клиентов на Upwork.

> ⚠️ Важно: сам Upwork **не присылает текст сообщения** в письме — только факт
> «такой-то клиент вам написал» и ссылку. Поэтому бот шлёт в чат: **кто написал**,
> **по какому проекту** и **кнопку «Открыть переписку в Upwork»** (прямая ссылка
> на нужный диалог). Это даёт моментальный пинг на телефон, чтобы не пропустить.
...
- раз в N минут (по умолчанию 10) скрипт ищет в почте новые письма
  по фильтру «**отправитель Upwork** + **тема `sent you a message`**»;
